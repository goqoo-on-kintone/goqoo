...
const dropbox = readFilePromise('dropbox.js', 'bin')
...
const goqooNew = async opts => {
  ...
  const app1Dir = path.join(projectName, 'apps', 'app1')
  mkdirp.sync(app1Dir)
  ...
  mkdirp.sync(binDirPath)

  // dropbox
  const dropboxPath = path.join(binDirPath, 'dropbox.js')
