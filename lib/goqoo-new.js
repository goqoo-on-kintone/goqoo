...
const goqooNew = async opts => {
  ...
  // Git初期化
  fs.writeFileSync(path.join(dirPath, '.gitignore'), await gitignore)
  ...
  // webpack初期設定
  const webpackConfigJsPath = path.resolve(path.join(dirPath, 'webpack.config.js'))
  ...
  // ESLint初期設定
  const eslintrcPath = path.resolve(path.join(dirPath, '.eslintrc.yml'))
  fs.writeFileSync(eslintrcPath, await eslintrc)
  console.log('.eslintrc.yml: created!')

  // Prettier初期設定
  const prettierrcPath = path.resolve(path.join(dirPath, '.prettierrc.yml'))
